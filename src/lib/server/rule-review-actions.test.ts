...
beforeEach(() => {
	...
	vi.mocked(updateRuleActiveForHousehold).mockResolvedValue({ error: null } as never);
...
describe('rule actions', () => {
	it('rejects invalid cross-household category relations before inserting', async () => {
		vi.mocked(validateTransactionRelations).mockResolvedValue('Categoria inválida para este grupo');
		const from = vi.fn();

		const result = await ruleActions.create(event(requestWith({
			pattern: 'MERCADO', pattern_type: 'description_contains', category_id: 'foreign-category'
		}), { from }));

		expect(result).toMatchObject({ status: 400, message: 'Categoria inválida para este grupo' });
		expect(from).not.toHaveBeenCalled();
	});

	it('surfaces rule persistence failures and sends the scoped rule payload', async () => {
		const insertion = new QueryMock({ error: { message: 'rule insert failed' } });
		const from = vi.fn(() => insertion);

		const result = await ruleActions.create(event(requestWith({
			pattern: 'MERCADO', pattern_type: 'description_contains', category_id: 'category-a', confidence: '0.8'
		}), { from }));

		expect(result).toMatchObject({ status: 500, message: 'rule insert failed' });
		expect(insertion.calls[0]).toEqual({ method: 'insert', args: [expect.objectContaining({
			household_id: 'household-a', created_by_user_id: 'user-a', category_id: 'category-a', active: true
		})] });
	});

	it('propagates scoped toggle persistence failures', async () => {
		vi.mocked(updateRuleActiveForHousehold).mockResolvedValue({ error: { message: 'toggle failed' } } as never);

		const result = await ruleActions.toggle(event(requestWith({ rule_id: 'rule-a', active: 'false' })));

		expect(result).toMatchObject({ status: 500, message: 'toggle failed' });
		expect(updateRuleActiveForHousehold).toHaveBeenCalledWith(expect.anything(), 'household-a', 'rule-a', false);
	});
});

describe('review action', () => {
	const fields = {
		transaction_id: 'tx-a', category_id: 'category-a', subcategory_id: 'subcategory-a', owner_profile_id: 'profile-a'
	};
	...
	it('does not persist an invalid relation', async () => {
		vi.mocked(validateTransactionRelations).mockResolvedValue('Perfil financeiro inválido para este grupo');

		const result = await reviewActions.default(event(requestWith(fields)));

		expect(result).toMatchObject({ status: 400, message: 'Perfil financeiro inválido para este grupo' });
		expect(updateTransactionForHousehold).not.toHaveBeenCalled();
	});

	it('surfaces update failures and never learns from an unpersisted adjustment', async () => {
		vi.mocked(updateTransactionForHousehold).mockResolvedValue({ error: { message: 'update failed' } } as never);
	...
	it('learns only after a successful household-scoped update', async () => {
		await expect(reviewActions.default(event(requestWith(fields)))).resolves.toMatchObject({ success: true });
		...
		expect(learnFromTransactionAdjustment).toHaveBeenCalledWith(expect.anything(), expect.objectContaining({
			householdId: 'household-a', userId: 'user-a', transactionId: 'tx-a'
		}));
	});
});
