...
import { confidenceForReinforcement, learnFromTransactionAdjustment } from './learning';
...
describe('learnFromTransactionAdjustment', () => {
	it('does not create a rule without a category', async () => {
		...
		await learnFromTransactionAdjustment(supabase.client, { ...learningInput, categoryId: null });

		expect(supabase.tables).toEqual([]);
	});

	it('does not learn from a transaction that is no longer available in the household', async () => {
		const supabase = supabaseForQueries([new QueryMock({ data: null, error: null })]);
	...
	it('creates a reusable description rule and removes installment markers', async () => {
		...
		const supabase = supabaseForQueries([transactionQuery, existingRuleQuery, insertQuery]);

		await learnFromTransactionAdjustment(supabase.client, learningInput);

		expect(supabase.tables).toEqual(['transactions', 'classification_rules', 'classification_rules']);
	...
	it('reinforces the same adjustment instead of creating a duplicate rule', async () => {
		...
		const supabase = supabaseForQueries([
			new QueryMock({ data: { ...transaction, merchant: 'Mercado Central' }, error: null }),
			new QueryMock({
				data: {
					id: 'rule-a',
					reinforcement_count: 1,
		...
		expect(updateQuery.calls).toContainEqual({
			method: 'update',
			args: [{ category_id: 'category-a', subcategory_id: null, owner_profile_id: null, active: true, reinforcement_count: 2, confidence: 0.72 }]
		});
	});
	...
	it('does not overwrite a conflicting rule after a passive confirmation', async () => {
		...
		await learnFromTransactionAdjustment(supabase.client, learningInput, 'confirmation');
