...
import { detectImageMimeType, extractRowsFromImage, extractRowsFromText } from './import-extract';
...
describe('AI extraction', () => {
	it('validates and normalizes transactions returned from pasted text', async () => {
		...
		const result = await extractRowsFromText('Compra na Loja Exemplo', 'credit_card', '2026-05');
	...
	it('sends images as data URLs and accepts default extraction values', async () => {
		mockedCallLlm.mockResolvedValue({ choices: [{ message: { content: '{}' } }] } as never);

		await expect(
			extractRowsFromImage(Buffer.from([1, 2, 3]), 'image/png', 'bank_account', '2026-05')
	...
	it('rejects invalid model payloads and handles provider failures safely', async () => {
		mockedCallLlm.mockResolvedValue({
			choices: [{ message: { content: '{"transactions":[{"date":"invalid","description":"X","amount":1}]}' } }]
		} as never);
		await expect(extractRowsFromText('conteúdo', 'bank_account', '2026-05')).resolves.toEqual({
			rows: [],
			confidence: 0,
