...
describe('category actions', () => {
	it('rejects unauthenticated writes before touching Supabase', async () => {
		const result = await actions.create_category(
			event(requestWith({ name: 'Viagens' }), false)
		);
	...
	it('surfaces category insert failures with household-scoped payload', async () => {
		...
		const result = await actions.create_category(
			event(requestWith({ name: '  Viagens  ' }))
		);
	...
	it('hides a shared/default category for only the current user', async () => {
		const category = new QueryMock({
			data: { id: 'category-a', parent_id: null, created_by_user_id: null },
			error: null
		});
		...
		await expect(
			actions.delete(event(requestWith({ category_id: 'category-a' })))
	...
	it('does not delete a personal category while it is referenced', async () => {
		const category = new QueryMock({
			data: { id: 'category-a', parent_id: null, created_by_user_id: 'user-a' },
			error: null
		});
		const usage = [1, 0, 0, 0, 0].map(
			(count) => new QueryMock({ count, error: null })
		);
		const queries = [category, ...usage];
		mockedFrom.mockImplementation(() => queries.shift() as never);

		const result = await actions.delete(
			event(requestWith({ category_id: 'category-a' }))
		);
		...
		expect(
			usage.every((query) =>
				query.calls.every((call) => call.method !== 'delete')
			)
		).toBe(true);
	});
