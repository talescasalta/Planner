...
function normalizeTransactionUpdate(
	data: ReturnType<typeof transactionUpdateSchema.parse>
) {
	return {
		...data,
...
export const load: PageServerLoad = async ({
	...
	const selectedCategoryIds = [
		enrichedTx.category_id,
		enrichedTx.subcategory_id
...
export const actions: Actions = {
	default: async ({
		...
		const { error } = await updateTransactionForHousehold(
			supabase,
			params.id,
			householdId,
			patch
		...
		await learnFromTransactionAdjustment(supabaseAdmin, {
			...
			subcategoryId: normalizedData.category_id
				? (normalizedData.subcategory_id ?? null)
				: null,
	...
	delete: async ({ params, locals: { supabase, safeGetSession } }) => {
		...
		const query = tx?.reference_month
			? `?month=${encodeURIComponent(tx.reference_month)}`
			: '';
