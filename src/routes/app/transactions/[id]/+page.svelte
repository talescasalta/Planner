...
<div class="max-w-2xl mx-auto space-y-6">
	<div class="flex items-center justify-between">
		...
		<a
			...
			class="text-sm text-gray-600 hover:text-gray-900">Voltar</a
		>
	</div>
	...
	>
		...
		<div>
			<label for="merchant" class="block text-sm font-medium text-gray-700"
				>Comerciante</label
			>
			<input
				id="merchant"
		...
		<div class="grid grid-cols-1 md:grid-cols-3 gap-4">
			<div>
				...
				<select
					...
					onchange={() => (selectedSubcategoryId = null)}
					class="mt-1 block w-full rounded-md border-gray-300 shadow-sm px-3 py-2 disabled:bg-gray-100"
				>
			...
			<div>
				<label
					for="subcategory_id"
					class="block text-sm font-medium text-gray-700">Subcategoria</label
				>
				<select
					id="subcategory_id"
					name="subcategory_id"
					disabled={!editable || !selectedCategoryId}
			...
			<div>
				<label
					for="owner_profile_id"
					class="block text-sm font-medium text-gray-700">Atribuir a</label
				>
				<select
					id="owner_profile_id"
		...
		<div class="grid grid-cols-1 md:grid-cols-2 gap-4">
			<div>
				<label
					for="paid_by_user_id"
					class="block text-sm font-medium text-gray-700">Pago por</label
				>
				<select
					id="paid_by_user_id"
				...
				>
					...
					{#each members as m (m.user_id)}
						<option
							value={m.user_id}
							selected={m.user_id === tx.paid_by_user_id}
							>{m.display_name ?? m.user_id}</option
						>
					{/each}
				</select>
			</div>
			...
			<div>
				<label
					for="split_method"
					class="block text-sm font-medium text-gray-700">Divisão</label
				>
				<select
					id="split_method"
					name="split_method"
				...
				>
					<option
						...
						selected={tx.split_method === 'income_proportional'}
						>Por renda</option
					>
					<option value="equal" selected={tx.split_method === 'equal'}
						>50/50</option
					>
				</select>
			</div>
		...
		<div>
			<label for="review_status" class="block text-sm font-medium text-gray-700"
				>Status de revisão</label
			>
			<input
				id="review_status"
				type="text"
				value={tx.review_status}
				disabled
				class="mt-1 block w-full rounded-md border-gray-300 shadow-sm px-3 py-2 disabled:bg-gray-100"
			/>
		</div>
