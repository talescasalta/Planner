...
async function hideCategoryForUser(
	householdId: string,
	userId: string,
	categoryId: string
) {
...
export const actions: Actions = {
	create_category: async ({
		...
		if (
			...
			return fail(409, {
				success: false,
				message: 'Essa categoria já existe no seu gabarito'
			});
		}
	...
	create_subcategory: async ({
		...
		if (!parentId || !name)
			return fail(400, {
				success: false,
				message: 'Informe categoria e subcategoria'
			});
		...
		if (
			...
			return fail(409, {
				success: false,
				message: 'Essa subcategoria já existe nessa categoria'
			});
		}
	...
	delete: async ({ request, locals: { supabase, safeGetSession } }) => {
		...
		if (!categoryId)
			return fail(400, { success: false, message: 'Categoria ausente' });
		...
		if (category.created_by_user_id !== user.id) {
			const { error } = await hideCategoryForUser(
				householdId,
				user.id,
				categoryId
			);
		...
		if (usageCount > 0) {
			return fail(409, {
				success: false,
				message:
					'Essa categoria está em uso. Reclassifique as transações/regras ou remova as subcategorias antes de deletar.'
			});
		}
