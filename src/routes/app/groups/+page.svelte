<script lang="ts">
	...
	type GroupTransaction = PageData['groups'][number]['activity']['transactions'][number];

	function visibleTransactions(groupId: string, list: GroupTransaction[]): GroupTransaction[] {
		...
		if (sort === 'desc') arr = [...arr].sort((a, b) => Number(b.amount) - Number(a.amount));
		else if (sort === 'asc') arr = [...arr].sort((a, b) => Number(a.amount) - Number(b.amount));
		return arr;
	}
	...
	function sortedContributions(groupId: string, list: PageData['groups'][number]['activity']['contributions']) {
		const mode = sortMode[groupId] ?? 'paid_desc';
		const arr = [...list];
		if (mode === 'paid_desc') arr.sort((a, b) => b.expense_total - a.expense_total);
		else if (mode === 'paid_asc') arr.sort((a, b) => a.expense_total - b.expense_total);
		else arr.sort((a, b) => a.display_name.localeCompare(b.display_name, 'pt-BR'));
		return arr;
	}

	function cycleSort(groupId: string) {
		const current = sortMode[groupId] ?? 'paid_desc';
		const next = current === 'paid_desc' ? 'paid_asc' : current === 'paid_asc' ? 'name' : 'paid_desc';
...
<div class="mx-auto max-w-5xl space-y-6">
	<div class="flex items-center justify-between">
		<div>
			<p class="text-sm font-medium uppercase tracking-wider text-gray-500">Grupos</p>
			<h2 class="mt-1 text-2xl font-semibold text-gray-950">Pessoas que dividem as finanças</h2>
		</div>
		<button
			type="button"
	...
	{#if form?.success && form?.message}
		<p class="rounded-md border border-emerald-200 bg-emerald-50 px-3 py-2 text-sm text-emerald-700">{form.message}</p>
	{/if}

	{#if showCreateForm}
		<form method="POST" action="?/create" use:enhance class="rounded-lg bg-white p-5 shadow">
			...
			<div class="mt-2 flex gap-2">
				...
				/>
				<button type="submit" class="rounded-md bg-emerald-600 px-4 py-2 text-sm font-medium text-white hover:bg-emerald-700">Criar</button>
			</div>
		</form>
	{/if}
	...
	{#if groups.length === 0 && !showCreateForm}
		<div class="rounded-lg bg-white p-8 text-center shadow">
			...
			<p class="mt-3 text-sm text-gray-700">Você ainda não pertence a nenhum grupo.</p>
			<p class="text-xs text-gray-500">Crie um grupo para começar a organizar suas finanças compartilhadas.</p>
		</div>
	{/if}

	{#each groups as group}
		{@const contributions = sortedContributions(group.id, group.activity.contributions)}
		{@const sortLabel = (sortMode[group.id] ?? 'paid_desc') === 'paid_desc' ? 'Mais pagou' : (sortMode[group.id] ?? '') === 'paid_asc' ? 'Menos pagou' : 'Nome'}
		<section class="space-y-4 rounded-lg bg-white p-5 shadow">
			<header class="flex flex-wrap items-end justify-between gap-3">
				<div>
					...
					<p class="text-xs text-gray-500">
						Seu papel: <span class="font-medium text-gray-700">{group.role === 'admin' ? 'Administrador' : 'Membro'}</span>
				...
				{#if group.activity.monthOptions.length > 0}
					<form method="GET" class="flex items-center gap-2">
						<label for={`month-${group.id}`} class="text-xs font-medium uppercase tracking-wider text-gray-500">Mês</label>
						<select
							id={`month-${group.id}`}
			...
			<div class="grid grid-cols-2 gap-3 sm:grid-cols-4">
				<div class="rounded-md bg-gray-50 p-3">
					<p class="text-[11px] uppercase tracking-wide text-gray-500">Transações</p>
					<p class="mt-1 text-lg font-semibold text-gray-900">{group.activity.summary.count}</p>
				</div>
				<div class="rounded-md bg-rose-50 p-3">
					<p class="text-[11px] uppercase tracking-wide text-rose-700">Despesas</p>
					<p class="mt-1 text-lg font-semibold text-rose-800">{formatCurrency(group.activity.summary.expenses)}</p>
				</div>
				<div class="rounded-md bg-emerald-50 p-3">
					<p class="text-[11px] uppercase tracking-wide text-emerald-700">Receitas</p>
					<p class="mt-1 text-lg font-semibold text-emerald-800">{formatCurrency(group.activity.summary.credits)}</p>
				</div>
				<div class="rounded-md bg-gray-50 p-3">
					<p class="text-[11px] uppercase tracking-wide text-gray-500">Saldo</p>
					<p class={`mt-1 text-lg font-semibold ${group.activity.summary.balance >= 0 ? 'text-emerald-800' : 'text-rose-800'}`}>
							...
							<li>
								<div class="flex items-center justify-between gap-3 text-sm">
									<div class="flex items-center gap-2 min-w-0">
										<span class="truncate font-medium text-gray-900">{c.display_name}</span>
										{#if c.user_id === data.user?.id}
											<span class="rounded-full bg-indigo-100 px-1.5 py-0.5 text-[10px] font-semibold text-indigo-700">Você</span>
										{/if}
			...
			<div class="border-t border-gray-100 pt-3">
				<div class="flex items-center justify-between">
					...
					{#if group.role === 'admin'}
						<button
							type="button"
							onclick={() => (openAddMemberFor = openAddMemberFor === group.id ? null : group.id)}
				...
				{#if openAddMemberFor === group.id}
					<form method="POST" action="?/add_member" use:enhance class="mt-3 flex gap-2">
						<input type="hidden" name="group_id" value={group.id} />
						<input
							name="email"
							type="email"
							...
							class="flex-1 rounded-md border-gray-300 px-3 py-2 text-sm shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
						/>
						<button type="submit" class="rounded-md bg-indigo-600 px-3 py-2 text-sm font-medium text-white hover:bg-indigo-700">Adicionar</button>
					</form>
				{/if}

				<ul class="mt-3 flex flex-wrap gap-2">
					{#each group.members as member}
						<li class="inline-flex items-center gap-1.5 rounded-full border border-gray-200 bg-gray-50 px-3 py-1 text-xs">
							<span class="font-medium text-gray-900">{member.display_name}</span>
							<span class="text-gray-500">·  {member.role === 'admin' ? 'Admin' : 'Membro'}</span>
							{#if member.user_id === data.user?.id}
								<span class="rounded-full bg-indigo-100 px-1.5 text-[10px] font-semibold text-indigo-700">Você</span>
							{:else if group.role === 'admin'}
								<form method="POST" action="?/remove_member" use:enhance class="inline">
									<input type="hidden" name="group_id" value={group.id} />
									<input type="hidden" name="user_id" value={member.user_id} />
									<button type="submit" class="rounded-full p-0.5 text-gray-400 hover:bg-red-100 hover:text-red-700" title="Remover">
										<X class="h-3 w-3" />
									</button>
								</form>
							{/if}
			...
			{#if group.activity.summary.count > 0}
				{@const txs = visibleTransactions(group.id, group.activity.transactions)}
				{@const sort = txAmountSort[group.id] ?? 'none'}
				<div class="border-t border-gray-100 pt-3">
					<div class="flex flex-wrap items-end justify-between gap-3">
						<h4 class="text-sm font-semibold text-gray-800">Transações ({txs.length}{txs.length !== group.activity.transactions.length ? ` de ${group.activity.transactions.length}` : ''})</h4>
					...
					{#if txs.length === 0}
						<p class="mt-3 text-xs text-gray-500">Nenhuma transação corresponde à busca.</p>
					{:else}
						<div class="mt-3 overflow-x-auto rounded-md border border-gray-100">
							<table class="min-w-full divide-y divide-gray-100 text-sm">
									...
									<tr>
										...
										<th class="px-3 py-2 text-right text-[11px] font-medium uppercase tracking-wider text-gray-500">
											<button
												...
												title={sort === 'none' ? 'Ordenar por valor' : sort === 'desc' ? 'Maior para menor' : 'Menor para maior'}
											>
												Valor
												{#if sort === 'desc'}
													<ArrowDown class="h-3.5 w-3.5" />
								...
								<tbody class="divide-y divide-gray-100">
									{#each txs as tx}
										<tr>
											<td class="whitespace-nowrap px-3 py-2 text-gray-700">{tx.date}</td>
											<td class="px-3 py-2">
