...
function readReviewClassification(formData: FormData) {
	...
	return {
		...
		subcategoryId: categoryId
			? String(formData.get('subcategory_id') ?? '') || null
			: null,
...
export const actions: Actions = {
	default: async ({ request, locals: { supabase, safeGetSession } }) => {
		...
		const { transactionId, categoryId, subcategoryId, ownerProfileId } =
			readReviewClassification(formData);

		if (!transactionId)
			return fail(400, { success: false, message: 'ID da transação ausente' });
		...
		if (!editable) {
			return fail(403, {
				success: false,
				message: 'Voce nao tem permissao para editar esta transacao.'
			});
		}
