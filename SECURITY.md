...
3. **Testes**: `npm test` — os testes de `src/lib/server/security.test.ts` cobrem o escopo por household; adicione casos ao tocar em autorização. Para policies e RPCs, rode também os testes de RLS (`npx supabase db start && npm run test:rls`) e adicione asserções em `supabase/tests/` ao criar tabela, policy ou função `SECURITY DEFINER` nova.
...
- **Testes de RLS (pgTAP)**: `supabase/tests/rls_security.test.sql` executa as policies reais num Postgres local e afirma os três invariantes — `anon` não lê nenhuma das tabelas públicas, membro de um household não lê nem escreve dados de outro household, e as RPCs `SECURITY DEFINER` restritas negam `anon`/`authenticated` (catálogo e chamada real). O workflow `RLS Tests` roda em PRs que tocam `supabase/**`, na `main` e semanalmente; local: `npx supabase db start && npm run test:rls`. O check estático de migrations valida o texto; estes testes validam o comportamento — mudanças em policies devem sempre passar pelos dois.
- `quality:deadcode` (knip) reporta arquivos, exports e dependências não usados. É informativo, não bloqueia; achados devem ser triados periodicamente.
