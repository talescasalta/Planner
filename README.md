...
5. Review classifications in **Transações**. Every row shows compact always-visible dropdowns for category, dependent subcategory (with inline "create new"), and assignment — changing any of them saves that row immediately, spreadsheet style. Select multiple rows with the checkboxes to apply the same classification to all of them in one submit. Filter the list by source (**Cartão de crédito** / **Conta corrente**), category, subcategory, and review status (needs review / confirmed / ignored); free-text search and amount sorting operate on the loaded month. Manual category/subcategory corrections are saved as personal rules and increase confidence over repeated reinforcements (`reinforcement_count`), so future imports classify automatically once a merchant is confirmed enough times.
